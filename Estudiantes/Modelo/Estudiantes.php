...
class Estudiantes extends Conexion {
    ...
    public function update( $Id, $Nombre, $Apellido, $Documento, $Correo, $Materia, $Docente, $Promedio, $Fecha ) {
        $statement = $this->db->prepare( "UPDATE estudiantes SET NOMBRE =:Nombre, APELLIDO = :Apellido,
    ...
    public function search($Search){
        $rows=null;
        $statement= $this->db->prepare("SELECT * FROM estudiantes WHERE NOMBRE LIKE concat('%',:Search,'%') OR
        APELLIDO LIKE concat('%',:Search,'%') OR DOCUMENTO LIKE concat('%','Search','%') OR CORREO LIKE concat('%','Search','%')
        OR MATERIA LIKE concat('%','Search','%') OR DOCENTE LIKE concat('%','Search','%')");
